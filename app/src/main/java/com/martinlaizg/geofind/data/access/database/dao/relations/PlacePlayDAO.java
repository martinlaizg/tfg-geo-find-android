...
public interface PlacePlayDAO {
	...
	@Query("SELECT * FROM places INNER JOIN place_play as join_pp ON places.id=join_pp.place_id WHERE join_pp.play_id = :play_id")
	List<Place> getPlacesByPlay(int play_id);


	@Query("SELECT * FROM plays INNER JOIN place_play as join_pp ON plays.id=join_pp.play_id WHERE join_pp.place_id = :place_id")
